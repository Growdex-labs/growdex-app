...
import { AiCampaignWorkspace } from "./AiCampaignWorkspace";
import { AudienceStrategyIdentityCard } from "./AudienceStrategyIdentityCard";
import type { AiMessage } from "./AiSidePanel";
import { AdCreatedModal } from "./AdCreatedModal";
import type { AiStep, AiStepStatus } from "./use-ai-campaign-flow";
import { useAiCampaignFlow } from "./use-ai-campaign-flow";
import { AudienceTargetingScreen } from "./AudienceTargetingScreen";
...
import { ManualGoalScreen } from "./ManualGoalScreen";
import { ManualPlatformScreen } from "./ManualPlatformScreen";
import {
  CreateMethodBox,
  type CreationMethod,
...
export function CampaignSetupWorkspace({
  ...
  const editCampaignId = campaignId ?? searchParams.get("id");
  const isLiveEdit = mode === "live";
  ...
  const [campaign, setCampaign] = useState<CreateCampaignPayload>(() =>
    createInitialCampaignPayload(countryCurrency),
  );
  ...
  useEffect(() => {
    setCampaign((current) => {
      const audienceStrategies = current.audienceStrategies.map((strategy) =>
        ...
        strategy.budget.currency !== budgetCurrency
          ? { ...strategy, budget: { ...strategy.budget, currency: budgetCurrency } }
          : strategy,
      );
      return audienceStrategies.some(
        (strategy, index) => strategy !== current.audienceStrategies[index],
      )
        ? { ...current, audienceStrategies }
        : current;
    });
  }, [budgetCurrency]);
  ...
  }, [campaign, editingCampaignLoading, isLiveEdit, method]);
  ...
  }, [editAdIndex, editCampaignId, editStrategyId, isLiveEdit]);
  ...
  const uploadMedia = async (
    ...
    if (
      ...
      setError("TikTok images must be JPG, JPEG, or PNG files.");
      return;
    }
  ...
  const saveLiveChanges = async () => {
    ...
    try {
      ...
      setError(
        failure instanceof Error
          ? failure.message
          : "Could not save the live campaign.",
      );
      setPublishing(false);
    }
  };
  ...
  const stepper = (
    <CampaignStepper
      ...
      onStepClick={(index) =>
        (isLiveEdit ? index > 0 : index <= step) && setStep(index)
      }
    />
  );
                ...
                <div className="mx-auto max-w-5xl p-4 md:p-8">
                  ...
                  {step === 7 && (
                    <ReviewPublishScreen
                      ...
                      onEditStrategy={openStrategyEditor}
                      onSaveDraft={
                        isLiveEdit ? undefined : () => void createDraft()
                      }
                      onPublish={() =>
                        isLiveEdit
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
              ...
              <p className="mt-2 text-sm text-gray-600">
                Growdex will update the live campaign on Meta or TikTok. Spend
                continues unless you pause it.
              </p>
              <div className="mt-6 flex justify-end gap-3">
                <button
                  type="button"
                  disabled={publishing}
                  onClick={() => setConfirmLiveSave(false)}
                  className="rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-gilroy-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  type="button"
                  disabled={publishing}
                  onClick={() => void saveLiveChanges()}
                  className="rounded-lg bg-khaki-200 px-4 py-2.5 text-sm font-gilroy-medium text-gray-900 hover:bg-khaki-300 disabled:opacity-50"
                >
